...
class StreamProcessingSpec extends AnyWordSpec with Matchers {
  ...
  "Difference in currencies" should {
    "be correct if compare == true" in {
      val today = LocalDate.now()
      val source = Source(Seq(
    ...
    "be correct if compare == false" in {
      val today = LocalDate.now()
      val source = Source(Seq(
        Entry(today, Map(EUR -> 15D, USD -> 14D)),
        Entry(today.plusDays(1), Map(EUR -> 22D, USD -> 20D)),
